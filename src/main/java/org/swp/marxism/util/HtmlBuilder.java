	...
	private String getDataContent(Meeting meeting) {
		...
		for(Theme theme : meeting.getThemes()) {
			...
			for(Meeting m : theme.getMeetings()) {
				builder.append("<div class=\"meetings__item__title\">" + m.getTitle() + "</div>");
				builder.append("<div class=\"meetings__item__time\">" + m.getDay() + " " + m.getTime() + "</div>");
			}
		}
